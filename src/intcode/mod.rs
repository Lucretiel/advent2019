...
pub fn binary_operation(compute: impl Fn(isize, isize) -> isize) -> impl FnMut(&mut Machine) {
    chain(
...
pub fn step_output(
    ...
    let mut op_input = chain(set_with(use_input(input), param(1)), advance_ip(2));
    ...
    // Jump if true
    let mut op_jmp_true = set_ip(cond_address(
        param(1).map(|c| c != 0),
        param(2).deref(),
        IP.offset(3),
    ));

    let op_jmp_false = set_ip(cond_address(
        param(1).map(|c| c == 0),
        param(2).deref(),
        IP.offset(3),
    ));
